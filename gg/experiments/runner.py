...
Options:
  ...
  --max-depth N         How deep to go before stopping splitting [default: 4]
...
class RunInputs(object):
    def __init__(self, net, prop, infra, jobs, initial_divides, divides, timeout, initial_timeout, timeout_factor, divide_strategy, trial = 0, max_depth=4):
    ...
    def setup_gg(self):
        gg_py_script = abs_marabou_repo() + "/gg/marabou.py"
        this_data_dir = self.run_data_dir()
        ...
        env['PATH'] = env['PATH'] + ":" + abs_marabou_repo() + "/build"
        print(' '.join(args))
        sub.run(args, env = env, cwd = this_data_dir, check = True)
